...
from .geospatial import build_grid, add_distance_columns, h3_to_geodataframe
...
def main(generate_map: bool, config: str):
    ...
    try:
        ...
        click.echo(f"Found {len(rock_a)} {settings.rock_a} polygons and {len(rock_b)} {settings.rock_b} polygons")
        ...
        if generate_map:
            ...
            fig = build_static_map(scored_gdf)  # Use the GeoDataFrame for the map
